  ...
  <div>
    ...
    <div>
      <el-row style="margin-top:1%">
        ...
        <el-col :span="6">
          ...
          >
            <!--搜索框  -->
            <el-form-item label="标题" prop="name" style="margin:0px">
    ...
    >
      ...
      >
        ...
        <el-form-item label="发布人" prop="pubren">
          <el-input v-model="editForm.pubren" disabled></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.normal.basic.notice.search;
      await that.$http
    ...
    async MinusConfirm(row) {
      // console.log(row);
      let that = this;
      const url = that.APIurl.API.api.normal.basic.notice.delete;
      await that.$http
        .delete(url + row.id)
        .then(function(response) {
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.normal.basic.notice.get;
      await that.$http
        .get(url, {
          params: that.page,
        })
    ...
    async UrlSslou() {
      const that = this;
      const url = that.APIurl.API.api.normal.basic.notice.sslou;
      await that.$http
        .get(url)
        .then(function(response) {
          // console.log(response);
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.normal.basic.notice.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.normal.basic.notice.put;
      await that.$http
