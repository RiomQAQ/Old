<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>出入信息管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- <p>hello</p> -->
    <!-- 表单之上 -->
    <div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="来访日期" prop="date">
          <el-date-picker
            type="date"
            placeholder="选择日期"
        ...
        <el-form-item label="去何地" prop="go">
          <el-input v-model="addForm.go" style="width:251px"></el-input>
        </el-form-item>
    ...
    >
      ...
      >
        ...
        <el-form-item label="去何地" prop="go">
          <el-input v-model="editForm.go" style="width:251px"></el-input>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input
            type="textarea"
...
export default {
  ...
  methods: {
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.access.search;
      await that.$http
    ...
    async MinusConfirm(row) {
      console.log(row);
      let that = this;
      const url = that.APIurl.API.api.access.delete;
      await that.$http
        .delete(url + row.id)
        .then(function(response) {
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.access.get;
      await that.$http
        .get(url, {
          params: that.page,
        })
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.access.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.access.put;
      await that.$http
