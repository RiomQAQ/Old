  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="姓名" prop="name">
          <!-- <el-input v-model="addForm.name"></el-input> -->
          <el-select v-model="addForm.name" filterable>
            <el-option
              v-for="item in elder"
              :key="item.id"
              :value="item.name"
              :label="item.name"
              @click.native="Changeidcard(item.idcardno)"
            >
        ...
        <el-form-item label="检查时间" prop="jcdate">
          <el-date-picker
            type="date"
            placeholder="选择日期"
            v-model="addForm.jcdate"
            style="width: 251px"
            value-format="yyyy-MM-dd"
          ></el-date-picker>
        </el-form-item>
        <el-form-item label="血脂" prop="xuezhi">
          <el-input v-model="addForm.xuezhi"></el-input>
        </el-form-item>
        <el-form-item label="血糖" prop="xuetang">
          <el-input v-model="addForm.xuetang"></el-input>
        </el-form-item>
        <el-form-item label="收缩血压" prop="ssxy">
          <el-input v-model="addForm.ssxy"></el-input>
        </el-form-item>
        <el-form-item label="舒张血压" prop="szxy">
          <el-input v-model="addForm.szxy"></el-input>
        </el-form-item>
        <el-form-item label="体重" prop="weight">
          <el-input v-model="addForm.weight"></el-input>
        </el-form-item>
        <el-form-item label="尿酸" prop="niaosuan">
          <el-input v-model="addForm.niaosuan"></el-input>
        </el-form-item>
      </el-form>
    ...
    >
      ...
      >
        ...
        <el-form-item label="身份证号" prop="idcardno">
          <el-input v-model="editForm.idcardno" disabled></el-input>
        </el-form-item>
        <el-form-item label="检查时间" prop="jcdate">
          <el-date-picker
            type="date"
            placeholder="选择日期"
            v-model="editForm.jcdate"
            style="width: 251px"
            value-format="yyyy-MM-dd"
          ></el-date-picker>
        </el-form-item>
        <el-form-item label="血脂" prop="xuezhi">
          <el-input v-model="editForm.xuezhi"></el-input>
        </el-form-item>
        <el-form-item label="血糖" prop="xuetang">
          <el-input v-model="editForm.xuetang"></el-input>
        </el-form-item>
        <el-form-item label="收缩血压" prop="ssxy">
          <el-input v-model="editForm.ssxy"></el-input>
        </el-form-item>
        <el-form-item label="舒张血压" prop="szxy">
          <el-input v-model="editForm.szxy"></el-input>
        </el-form-item>
        <el-form-item label="体重" prop="weight">
          <el-input v-model="editForm.weight"></el-input>
        </el-form-item>
        <el-form-item label="尿酸" prop="niaosuan">
          <el-input v-model="editForm.niaosuan"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  computed: {},
  methods: {
    Changeidcard(idcardno) {
      this.addForm.idcardno = idcardno;
      // console.log(vId);
    },
    async getEldername() {
      const that = this;
      const url = that.APIurl.API.api.normal.old.health.elder;
      await that.$http
        .get(url)
        .then(function(response) {
          // console.log(response);
          // 获取数据
          that.elder = response.data.data;
          // console.log(elder);
          // that.total = response.total;
          that.$message.success("获取老人信息成功");
        })
        .catch(function(error) {
          //请求失败处理
          that.$message.error("获取老人信息失败");
        });
    },
    // 点击搜索按钮
    Search() {
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.health.search;
      await that.$http
    ...
    async MinusConfirm(row) {
      // console.log(row);
      let that = this;
      const url = that.APIurl.API.api.normal.old.health.delete;
      await that.$http
        .delete(url + row.id)
        .then(function(response) {
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.health.get;
      await that.$http
        .get(url, {
          params: that.page,
        })
    ...
    async UrlSslou() {
      const that = this;
      const url = that.APIurl.API.api.normal.old.health.sslou;
      await that.$http
        .get(url)
        .then(function(response) {
          // console.log(response);
          that.sslou = response.data;
        })
        .catch(function(error) {
          //请求失败处理
          console.log(error);
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.health.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.health.put;
      await that.$http
