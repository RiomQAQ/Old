<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>来访管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- <p>hello</p> -->
    <!-- 表单之上 -->
    <div>
    ...
    >
      ...
      >
        <el-form-item label="姓名" prop="ename">
          ...
          <el-select v-model="addForm.ename" filterable>
            <el-option
              v-for="item in elder"
              :key="item.id"
              :value="item.name"
        ...
        <el-form-item label="被访人编号" prop="targetid">
          <el-input v-model="addForm.targetid" disabled></el-input>
        </el-form-item>

        <el-form-item label="来访时间" prop="creattime">
          <el-date-picker
            type="date"
            placeholder="选择日期"
        ...
        <el-form-item
          ...
          label-width="110px"
        >
          <el-input v-model="addForm.lfrcardno"></el-input>
        </el-form-item>
    ...
    >
      ...
      >
        ...
        <el-form-item label="被访姓名" prop="ename">
          <el-input v-model="editForm.ename" disabled></el-input>
        </el-form-item>
        <el-form-item label="被访编号" prop="targetid">
          <el-input v-model="editForm.targetid" disabled></el-input>
        </el-form-item>
        ...
        <el-form-item
          ...
          label-width="110px"
        >
          <el-input v-model="editForm.lfrcardno" style="width:251px"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.laifang.search;
      await that.$http
    ...
    async MinusConfirm(row) {
      // console.log(row);
      let that = this;
      const url = that.APIurl.API.api.normal.old.laifang.delete;
      await that.$http
        .delete(url + row.id)
        .then(function(response) {
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.laifang.get;
      await that.$http
        .get(url, {
          params: that.page,
        })
    ...
    async UrlSslou() {
      const that = this;
      const url = that.APIurl.API.api.normal.old.laifang.sslou;
      await that.$http
        .get(url)
        .then(function(response) {
          // console.log(response);
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.laifang.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.laifang.put;
      await that.$http
