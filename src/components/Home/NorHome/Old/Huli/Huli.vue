      ...
      >
        ...
        <el-form-item label="月份" prop="yuefen">
          <el-date-picker
            type="month"
            placeholder="选择月份"
            v-model="addForm.yuefen"
            value-format="yyyy-MM"
            style="width: 251px"
          ></el-date-picker>
        </el-form-item>
        <el-form-item label="护理费" prop="hlfee">
          <el-input v-model.number="addForm.hlfee"></el-input>
        </el-form-item>
        <el-form-item label="床位费" prop="cwfee">
          <el-input v-model.number="addForm.cwfee"></el-input>
        </el-form-item>
        <el-form-item label="总费用" prop="totalfee">
          <el-input
            v-model.number="addForm.totalfee"
            @mousemove.native="ChangeAdd"
          ></el-input>
        </el-form-item>
      </el-form>
      ...
      >
        ...
        <el-form-item label="月份" prop="yuefen">
          <el-date-picker
            type="month"
            placeholder="选择月份"
            v-model="editForm.yuefen"
            value-format="yyyy-MM"
            style="width: 251px"
          ></el-date-picker>
        </el-form-item>
        <el-form-item label="护理费" prop="hlfee">
          <el-input v-model.number="editForm.hlfee"></el-input>
        </el-form-item>
        <el-form-item label="床位费" prop="cwfee">
          <el-input v-model.number="editForm.cwfee"></el-input>
        </el-form-item>
        <el-form-item label="总费用" prop="totalfee">
          <el-input
            v-model.number="editForm.totalfee"
            @mousemove.native="ChangeAdd"
          ></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    ChangeAdd() {
      this.editForm.totalfee = this.editForm.hlfee + this.editForm.cwfee;
    },
    // 点击搜索按钮
    Search() {
      // 校验name是否符合规则
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.huli.search;
      await that.$http
    ...
    async MinusConfirm(row) {
      // console.log(row);
      let that = this;
      const url = that.APIurl.API.api.normal.old.huli.delete;
      await that.$http
        .delete(url + row.id)
        .then(function(response) {
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.huli.get;
      await that.$http
        .get(url, {
          params: that.page,
        })
    ...
    async UrlSslou() {
      const that = this;
      const url = that.APIurl.API.api.normal.old.huli.sslou;
      await that.$http
        .get(url)
        .then(function(response) {
          // console.log(response);
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.huli.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.normal.old.huli.put;
      await that.$http
