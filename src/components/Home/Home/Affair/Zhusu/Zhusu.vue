    ...
    >
      ...
      >
        ...
        <el-form-item label="宿舍楼" prop="sslou">
          <el-select v-model="editForm.sslou" placeholder="请选择">
            <el-option
              v-for="item in sslou"
              :key="item.id"
              :label="item.louname"
...
export default {
  ...
  methods: {
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.admin.affair.zhusu.search;
      await that.$http
    ...
    async MinusConfirm(row) {
      // console.log(row);
      let that = this;
      const url = that.APIurl.API.api.admin.affair.zhusu.delete;
      await that.$http
        .delete(url + row.id)
        .then(function(response) {
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.admin.affair.zhusu.get;
      await that.$http
        .get(url, {
          params: that.page,
        })
    ...
    async UrlSslou() {
      const that = this;
      const url = that.APIurl.API.api.admin.affair.zhusu.sslou;
      await that.$http
        .get(url)
        .then(function(response) {
          // console.log(response);
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.admin.affair.zhusu.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.admin.affair.zhusu.put;
      await that.$http
