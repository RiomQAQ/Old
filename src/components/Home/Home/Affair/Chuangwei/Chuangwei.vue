<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>床位分配</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- <p>hello</p> -->
    <!-- 表单之上 -->
    <div>
      ...
      <div>
        ...
        >
          ...
          <el-table-column
            ...
            width="200"
          ></el-table-column>
          <el-table-column prop="zhusu" label="住宿情况" align="center">
            <template slot-scope="scope">
              <span style="margin-left: 10px" v-if="scope.row.chuangwei"
                >已分配床位</span
              >
              <span style="margin-left: 10px" v-if="!scope.row.chuangwei"
                >暂时没分配床位</span
    ...
    >
      ...
      >
        ...
        <el-form-item label="宿舍楼" prop="sslou">
          <el-input v-model="editForm.sslou"></el-input>
        </el-form-item>
        <el-form-item label="房间号" prop="fjno">
          <el-input v-model="editForm.fjno"></el-input>
        </el-form-item>
        <el-form-item label="床位" prop="chuangwei">
          <el-input v-model="editForm.chuangwei"></el-input>
        </el-form-item>
        <el-form-item label="入住时间" prop="rzdate">
          <el-date-picker
            type="date"
            placeholder="选择日期"
            v-model="editForm.rzdate"
            style="width: 251px"
...
export default {
  ...
  methods: {
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.admin.affair.chuangwei.search;
      await that.$http
    ...
    async MinusConfirm(row) {
      console.log(row);
      let that = this;
      const url = that.APIurl.API.api.admin.affair.chuangwei.delete;
      await that.$http
        .post(url + row.eid)
        .then(function(response) {
          that.$message.success("删除编号为" + row.eid + "的数据");
          that.getTableData();
        })
        .catch(function(error) {
          //请求失败处理
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.admin.affair.chuangwei.get;
      await that.$http
        .get(url)
        .then(function(response) {
          console.log(response);
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.admin.affair.chuangwei.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.admin.affair.chuangwei.put;
      await that.$http
