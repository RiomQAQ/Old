<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>紧急联系人管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- <p>hello</p> -->
    <!-- 表单之上 -->
    <div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="状态" prop="bmid">
          <el-input v-model="addForm.bmid"></el-input>
        </el-form-item>
      </el-form>
    ...
    >
      ...
      >
        ...
        <el-form-item label="身份证号" prop="number">
          <el-input v-model="editForm.number" style="width:251px"></el-input>
        </el-form-item>
        <el-form-item label="联系姓名" prop="cname">
          <el-input v-model="editForm.cname"></el-input>
        </el-form-item>
        <el-form-item label="联系性别" prop="csex">
          <el-select v-model="editForm.csex">
            <el-option label="男" value="男"></el-option>
            <el-option
              label="女"
              value="女"
        ...
        <el-form-item label="关系" prop="crelation">
          <el-select v-model="editForm.crelation" filterable>
            <el-option
              v-for="(value, key) in relation"
              :key="key"
              :label="value"
              :value="value"
              :value-key="value"
        ...
        <el-form-item label="联系地址" prop="caddress">
          <el-input v-model="editForm.caddress"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.contact.search;
      await that.$http
    ...
    async MinusConfirm(row) {
      // console.log(row);
      let that = this;
      const url = that.APIurl.API.api.contact.delete;
      await that.$http
        .delete(url + row.id)
        .then(function(response) {
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.contact.get;
      await that.$http
        .get(url, {
          params: that.page,
        })
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.contact.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.contact.put;
      await that.$http
