  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="籍贯" prop="jiguan">
          <el-select v-model="addForm.jiguan" filterable>
            <el-option
              v-for="(value, key) in native"
              :key="key"
              :label="value"
              :value="value"
              :value-key="value"
        ...
        <el-form-item label="民族" prop="nation">
          <el-select v-model="addForm.nation" filterable>
            <el-option
        ...
        <el-form-item label="部门" prop="bmid">
          <el-select v-model="addForm.bmid" placeholder="请选择">
            <el-option
              v-for="item in bumen"
              :key="item.id"
              :label="item.name"
    ...
    >
      ...
      >
        ...
        <el-form-item label="籍贯" prop="jiguan">
          <el-select v-model="editForm.jiguan" filterable>
            <el-option
              v-for="(value, key) in native"
              :key="key"
              :label="value"
        ...
        <el-form-item label="民族" prop="nation">
          <el-select v-model="editForm.nation" filterable>
            <el-option
              v-for="(value, key) in nation"
              :key="key"
              :label="value"
        ...
        <el-form-item label="部门" prop="bmid">
          <el-select v-model="editForm.bmid" placeholder="请选择">
            <el-option
              v-for="item in bumen"
              :key="item.id"
              :label="item.name"
...
<script>
import { searchrule, addrule, editrule, nation, native } from "./rule";
export default {
  data() {
    return {
      // 搜索表单
  ...
  methods: {
    async getBumen() {
      let that = this;
      const url = that.APIurl.API.api.admin.basic.yuangong.bumen;
      await that.$http
        .get(url)
        .then(function(response) {
          that.bumen = response.data;
          // console.log(that.bumen);
          that.$message.success("获取部门信息成功");
        })
        .catch(function(error) {
          //请求失败处理
          that.$message.error("获取部门信息失败");
        });
    },
    // 点击搜索按钮
    Search() {
    ...
    async UrlSearch() {
      let that = this;
      const url = that.APIurl.API.api.admin.basic.yuangong.search;
      // console.log(url);
      await that.$http
    ...
    async MinusConfirm(row) {
      // console.log(row);
      let that = this;
      const url = that.APIurl.API.api.admin.basic.yuangong.delete;
      await that.$http
        .delete(url + row.id)
        .then(function(response) {
    ...
    async UrlTableData() {
      let that = this;
      const url = that.APIurl.API.api.admin.basic.yuangong.get;
      await that.$http
        .get(url, {
          params: that.page,
        })
    ...
    async UrlAdd() {
      let that = this;
      const url = that.APIurl.API.api.admin.basic.yuangong.post;
      await that.$http
    ...
    async UrlEdit() {
      let that = this;
      const url = that.APIurl.API.api.admin.basic.yuangong.put;
      await that.$http
