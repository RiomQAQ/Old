...
// 获取条件查询
router.get("/manager/notice/search", function(req, res, next) {
...
// 获取列表所有数据
router.get("/manager/notice", function(req, res, next) {
...
//添加列表数据
router.post("/manager/notice", function(req, res, next) {
  var info = req.body;
  pool.getConnection(function(err, connection) {
    //先判断该账号是否存在
    var $sql = Url.noticepost(info);
    console.log($sql);
    connection.query($sql, function(err, result) {
      if (err) return console.log(err);
...
// 更新列表数据
router.put("/manager/notice", function(req, res, next) {
  var info = req.body;
  pool.getConnection(function(err, connection) {
    //先判断该账号是否存在
    var $sql = Url.noticeput(info);
    console.log($sql);
    connection.query($sql, function(err, result) {
      if (err) return console.log(err);
...
// 删除列表数据
router.delete("/manager/notice/delete", function(req, res, next) {
  let { id } = req.query;
  pool.getConnection(function(err, connection) {
    //先判断该账号是否存在
    var $sql = Url.noticedelete(id);
    console.log($sql);
    connection.query($sql, function(err, result) {
      if (err) return console.log(err);
