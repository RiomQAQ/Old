...
// 获取条件查询
router.get("/manager/nor/info/search", function(req, res, next) {
  let { name = "", current = 1, size = 10 } = req.query;
  pool.getConnection(function(err, connection) {
    //先判断该账号是否存在
    var length = 0;
    if (name) {
      var $sql = Url.oldgetname(name, (current - 1) * size, size);
      connection.query(
      ...
      var $sql = Url.oldget((current - 1) * size, size);
    }
...
// 获取列表所有数据
router.get("/manager/nor/info", function(req, res, next) {
